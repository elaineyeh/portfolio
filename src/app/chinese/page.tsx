  ...
  return (
    <main className="flex flex-col min-h-[100dvh] space-y-10">
      ...
      <section id="work">
        <div className="flex min-h-0 flex-col gap-y-3">
          <BlurFade delay={BLUR_FADE_DELAY * 5}>
            <h2 className="text-xl font-bold">接案經歷</h2>
          </BlurFade>
          {DATA.freelance.map((freelance, id) => (
            <BlurFade
              key={freelance.company}
              delay={BLUR_FADE_DELAY * 6 + id * 0.05}
            >
              <ResumeCard
                key={freelance.company}
                logoUrl={freelance.logoUrl}
                altText={freelance.company}
                title={freelance.company}
                subtitle={freelance.title}
                href={freelance.href}
                badges={freelance.badges}
                period={`${freelance.start} - ${freelance.end ?? "Present"}`}
            ...
            {DATA.skills.map((row, rowIndex) => (
              <div key={rowIndex} className="flex flex-wrap gap-1">
                {row.map((skill, skillIndex) => (
                  <BlurFade key={skill} delay={BLUR_FADE_DELAY * 10 + skillIndex * 0.05}>
                    <Badge>{skill}</Badge>
                  </BlurFade>
                ))}
              </div>
            ...
            <div className="flex flex-col items-center justify-center space-y-4 text-center">
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed">
                  曾參與過各種專案，從簡單的網站到複雜的網路應用程式，以下是一些我喜愛的專案。
                </p>
              </div>
            </div>
          </BlurFade>
